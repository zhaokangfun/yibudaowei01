package com.rk.entity;

import java.io.Serializable;


public class Qianmingb   implements Serializable {

    private Integer qmid;

    private Integer zcaoyid;

    private String shqm;

    private String dpqm;

    private String ysqm;

    private String hdqm;

    private String fyqm;

    public Integer getQmid() {
        return qmid;
    }

    public void setQmid(Integer qmid) {
        this.qmid = qmid;
    }

    public Integer getZcaoyid() {
        return zcaoyid;
    }

    public void setZcaoyid(Integer zcaoyid) {
        this.zcaoyid = zcaoyid;
    }

    public String getShqm() {
        return shqm;
    }

    public void setShqm(String shqm) {
        this.shqm = shqm;
    }

    public String getDpqm() {
        return dpqm;
    }

    public void setDpqm(String dpqm) {
        this.dpqm = dpqm;
    }

    public String getYsqm() {
        return ysqm;
    }

    public void setYsqm(String ysqm) {
        this.ysqm = ysqm;
    }

    public String getHdqm() {
        return hdqm;
    }

    public void setHdqm(String hdqm) {
        this.hdqm = hdqm;
    }

    public String getFyqm() {
        return fyqm;
    }

    public void setFyqm(String fyqm) {
        this.fyqm = fyqm;
    }
}